...
def legit_space (tetriminos,grid):
        ...
        a_posit= [[(j, i) for j in range(10) if grid[i][j] == (0,0,0)] for i in range(20)] #can't add in position if there is a tetrimnio--> AKA another color!
        #make list 1D, taking all positions in list+ making 1D, overriding list we had previously! Easier to loops through
        a_posit = [j for sub in a_posit for j in sub]
...
def window_draw (surface): 
        ...
        surface.blit(title_label, (upper_left_x + play_width/2 - (title_label.get_width()/2), 30)) #will center the text in the middle of the X axis, malleable if I were to change the screen size. y is fixed at the top of the screen, so just choose a value + see?
        ...
        pygame.draw.rect(surface, (128,128,128), (upper_left_x, upper_left_y, play_width, play_height),5) #grey rectangle that surrounds the border of the grid!
...
def next_tetrimino_display(tetrimino, surface):
        ...
        for i, line in enumerate(layout): 
            ...
            for j,column in enumerate(horizont):
                if column == '0':
                    pygame.draw.rect(surface, tetrimino.color, (upper_left_x+play_width+50+j*block_size, upper_left_y+play_height/2-100+i*block_size, block_size, block_size), 0)

        surface.blit(label, (upper_left_x+play_width+60, upper_left_y+play_height/2-70))
